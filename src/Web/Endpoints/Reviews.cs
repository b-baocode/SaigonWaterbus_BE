    ...
    public static void Map(RouteGroupBuilder group)
    {
        group.MapPost(CreateTripReview, "trips/{tripId:guid}")
            ...
            .WithDescription(OpenApiDescriptionBuilder.Build(
                ...
                "Review duoc hien thi ngay (status=Published), admin co the an neu vi pham."));
        ...
        group.MapGet(GetMyReviewableTrips, "my/reviewable-trips")
            ...
            .WithDescription(OpenApiDescriptionBuilder.Build(
                ...
                "myReview=null nghia la chua danh gia -> hien nut 'Danh gia'; nguoc lai hien noi dung da gui.",
        ...
        group.MapGet(GetAdminReviews, "admin")
            ...
            .WithDescription(OpenApiDescriptionBuilder.Build(
                "Admin hoac Manager",
                null,
                "Query (tat ca optional): status (Published|Hidden), rating (1-5), tripId, routeId, page, pageSize.",
                "Tra ve kem thong tin khach, bookingCode, tripCode, tuyen de doi soat."));

        group.MapPatch(UpdateReviewStatus, "{id:guid}/status")
            .RequireAuthorization()
            .WithSummary("Quan tri: an/hien mot danh gia")
            .WithDescription(OpenApiDescriptionBuilder.Build(
                "Admin hoac Manager",
                """{ "status": "Hidden" }""",
                "status: Published (hien) | Hidden (an khoi danh sach public). Khong xoa cung du lieu.",
