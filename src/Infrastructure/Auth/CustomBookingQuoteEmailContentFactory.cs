...
internal static class CustomBookingQuoteEmailContentFactory
{
    private static readonly CultureInfo ViCulture = CultureInfo.GetCultureInfo("vi-VN");

    public static string Subject(CustomBookingRequest request) =>
        $"Bao gia thue tau custom #{request.Id:N}"[..Math.Min($"Bao gia thue tau custom #{request.Id:N}".Length, 120)];
    ...
    private static void AppendBody(StringBuilder builder, CustomBookingRequest request, bool html)
    {
        ...
        AppendLine(builder, "Saigon Waterbus - Bao gia thue tau custom", html, strong: true);
        ...
        {
            AppendLine(builder, $"Tau: {request.PreferredVessel.Code} - {request.PreferredVessel.Name}", html);
        ...
        AppendLine(builder, "Thong tin bao gia", html, strong: true);
        AppendLine(builder, $"Tong tien: {Money(quote.QuotedPrice, quote.Currency)}", html);
        AppendLine(builder, $"Dat coc: {quote.DepositPercent.ToString("0.##", CultureInfo.InvariantCulture)}% = {Money(quote.DepositAmount, quote.Currency)}", html);
        ...
        {
            AppendLine(builder, $"Hieu luc den: {quote.ValidUntil.Value.ToOffset(TimeSpan.FromHours(7)):dd/MM/yyyy HH:mm} (GMT+7)", html);
        }

        AppendBreak(builder, html);
        AppendLine(builder, "Vui long kiem tra lai thong tin truoc khi xac nhan bao gia.", html);
    }
