...
public sealed class GmailCustomBookingConfirmationEmailSender : ICustomBookingConfirmationEmailSender
{
    private readonly IOptionsMonitor<GmailOptions> _optionsMonitor;
    private readonly ILogger<GmailCustomBookingConfirmationEmailSender> _logger;

    public GmailCustomBookingConfirmationEmailSender(
        IOptionsMonitor<GmailOptions> optionsMonitor,
        ILogger<GmailCustomBookingConfirmationEmailSender> logger)
    {
    ...
    public async Task SendConfirmationAsync(CustomBookingRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!options.Enabled)
        {
            _logger.LogInformation("Gmail is disabled. Skipping custom booking confirmation email for {RequestId}.", request.Id);
            return;
        }
        ...
        {
            _logger.LogWarning("Gmail Username/Password is not configured. Skipping custom booking confirmation email for {RequestId}.", request.Id);
            return;
        }
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Custom booking confirmation email sent by Gmail. RequestId: {RequestId}, Email: {Email}",
                request.Id,
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Gmail custom booking confirmation email failed. RequestId: {RequestId}, Email: {Email}",
                request.Id,
                request.ContactEmail);
        }
    }
