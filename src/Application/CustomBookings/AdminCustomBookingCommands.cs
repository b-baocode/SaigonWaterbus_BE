...
public sealed record GetAdminCustomBookingListQuery : IRequest<IReadOnlyList<AdminCustomBookingListItemDto>>;

public sealed class GetAdminCustomBookingListQueryHandler
    : IRequestHandler<GetAdminCustomBookingListQuery, IReadOnlyList<AdminCustomBookingListItemDto>>
{
    private readonly IApplicationDbContext _context;
    private readonly IUserContext _userContext;

    public GetAdminCustomBookingListQueryHandler(IApplicationDbContext context, IUserContext userContext)
    {
        _context = context;
        _userContext = userContext;
    }

    public async Task<IReadOnlyList<AdminCustomBookingListItemDto>> Handle(
        GetAdminCustomBookingListQuery request,
...
public sealed record GetAdminCustomBookingDetailQuery(Guid BookingId) : IRequest<CustomBookingDetailDto>;

public sealed class GetAdminCustomBookingDetailQueryHandler
    : IRequestHandler<GetAdminCustomBookingDetailQuery, CustomBookingDetailDto>
{
    private readonly IApplicationDbContext _context;
    private readonly IUserContext _userContext;

    public GetAdminCustomBookingDetailQueryHandler(IApplicationDbContext context, IUserContext userContext)
    {
        _context = context;
        _userContext = userContext;
    }
    ...
    public async Task<CustomBookingDetailDto> Handle(
        GetAdminCustomBookingDetailQuery request,
        CancellationToken cancellationToken)
    {
        await AuthSupport.EnsureCurrentUserIsAdminAsync(_context, _userContext, cancellationToken);
...
public sealed record UpdateCustomBookingStatusCommand(
    Guid BookingId,
    BookingStatus BookingStatus) : IRequest<CustomBookingDetailDto>;

public sealed class UpdateCustomBookingStatusCommandValidator
    : AbstractValidator<UpdateCustomBookingStatusCommand>
{
    public UpdateCustomBookingStatusCommandValidator()
    {
...
public sealed class UpdateCustomBookingStatusCommandHandler
    : IRequestHandler<UpdateCustomBookingStatusCommand, CustomBookingDetailDto>
{
    ...
    public async Task<CustomBookingDetailDto> Handle(
        UpdateCustomBookingStatusCommand request,
        CancellationToken cancellationToken)
    {
        await AuthSupport.EnsureCurrentUserIsAdminAsync(_context, _userContext, cancellationToken);
        ...
        switch (targetStatus)
        {
            case BookingStatus.PendingPayment:
                throw CreateStatusValidation(targetStatus,
                    "Custom booking không dùng trạng thái PendingPayment. Dùng PendingQuote khi chưa được admin chốt giá.");
            ...
            default:
                throw CreateStatusValidation(targetStatus, "Trạng thái custom booking không hợp lệ.");
        }
    ...
    private static void EnsureCanCancel(CustomBooking booking, BookingStatus targetStatus)
    {
        ...
        {
            throw CreateStatusValidation(targetStatus,
                "Không thể hủy custom booking đã hoàn tất hoặc đã hoàn tiền.");
        }
    }
    ...
    private static void EnsureCanExpire(CustomBooking booking, BookingStatus targetStatus)
    {
        ...
        {
            throw CreateStatusValidation(targetStatus,
                "Không thể chuyển custom booking đã xác nhận, đã hoàn tất hoặc đã hoàn tiền sang Expired.");
        }
    ...
    private static void EnsureCanComplete(CustomBooking booking, BookingStatus targetStatus)
    {
        ...
        {
            throw CreateStatusValidation(targetStatus,
                "Chỉ có thể hoàn tất custom booking đã thanh toán đủ.");
        }
    }
...
public sealed class QuoteCustomBookingCommandValidator : AbstractValidator<QuoteCustomBookingCommand>
{
...
public sealed class QuoteCustomBookingCommandHandler
    : IRequestHandler<QuoteCustomBookingCommand, QuoteCustomBookingResult>
{
    private const string PendingPaymentStatus = "Pending";
    private const string PaidPaymentStatus = "Paid";
    ...
    public async Task<QuoteCustomBookingResult> Handle(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            CustomBookingRoutePricingSupport.EnsureCanAutoPrice(rentalUnit, automaticPricing.RouteEstimate);
        }
        ...
        CancellationToken cancellationToken)
    {
        if (request.PromotionCode is not null)
        {
            return string.IsNullOrWhiteSpace(request.PromotionCode)
                ? null
                : await CustomBookingPricingSupport.ResolvePromotionAsync(
                    _context,
        ...
        CustomBookingPricingSupport.EnsurePromotionCanBeUsed(
            promotion,
            subtotalAmount,
            now,
            nameof(request.PromotionCode),
