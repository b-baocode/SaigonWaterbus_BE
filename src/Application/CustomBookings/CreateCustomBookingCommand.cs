...
public sealed class CreateCustomBookingCommandValidator : AbstractValidator<CreateCustomBookingCommand>
{
    ...
    private static bool HaveUniqueStopOrders(IReadOnlyList<CreateCustomBookingItineraryStopRequest>? stops) =>
        stops is null || stops.Select(x => x.StopOrder).Distinct().Count() == stops.Count;
}

public sealed class CreateCustomBookingCommandHandler
    : IRequestHandler<CreateCustomBookingCommand, CreateCustomBookingResult>
{
    ...
    public CreateCustomBookingCommandHandler(
    ...
    public async Task<CreateCustomBookingResult> Handle(
        CreateCustomBookingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.DepartureDate < minimumDepartureDate)
            throw new ValidationException([new ValidationFailure(nameof(request.DepartureDate),
                $"Custom booking phải được đặt trước ít nhất 7 ngày. Ngày khởi hành sớm nhất là {minimumDepartureDate:dd/MM/yyyy}.")]);
        ...
        var promotion = await CustomBookingPricingSupport.ResolvePromotionAsync(
            _context,
        ...
        return new CreateCustomBookingResult(
            booking.Id,
            booking.BookingCode,
            null,
    ...
    private async Task EnsureItineraryStationsExistAsync(
        IReadOnlyList<CreateCustomBookingItineraryStopRequest>? stops,
        CancellationToken cancellationToken)
    {
        ...
        if (missingStationId != Guid.Empty)
            throw new ValidationException([new ValidationFailure(nameof(CreateCustomBookingCommand.ItineraryStops),
                $"Điểm dừng có stationId '{missingStationId}' không tồn tại.")]);
    }
}

public sealed class CustomBookingPassengerRequestValidator : AbstractValidator<CustomBookingPassengerRequest>
{
    public CustomBookingPassengerRequestValidator()
    {
        RuleFor(x => x.FullName).NotEmpty().MaximumLength(150);
        RuleFor(x => x.DateOfBirth)
            .NotEmpty()
            .Must(x => CustomBookingPassengerSupport.TryParseDateOfBirth(x, out _))
            .WithMessage("Ngày sinh không hợp lệ. Dùng định dạng yyyy-MM-dd hoặc dd/MM/yyyy.");
        RuleFor(x => x.DateOfBirth)
            .Must(x => !CustomBookingPassengerSupport.TryParseDateOfBirth(x, out var dateOfBirth)
