...
public sealed record VerifyGooglePhoneRequest(string TempToken, string Otp);

public sealed class VerifyGooglePhoneRequestValidator : AbstractValidator<VerifyGooglePhoneRequest>
{
    public VerifyGooglePhoneRequestValidator()
    {
...
public sealed class VerifyGooglePhoneRequestUseCase
{
    ...
    public VerifyGooglePhoneRequestUseCase(
    ...
    public async Task<AuthSessionDto> ExecuteAsync(VerifyGooglePhoneRequest request, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw AuthSupport.CreateValidationException(nameof(VerifyGooglePhoneRequest.TempToken), "Số điện thoại đã được đăng ký.");
        }
        ...
        if (session.ExistingUserId.HasValue)
        {
            var existingUser = await _context.Set<User>()
                ...
                ?? throw AuthSupport.CreateValidationException(nameof(VerifyGooglePhoneRequest.TempToken), "User liên kết với temp token không còn tồn tại.");
        ...
        {
            throw AuthSupport.CreateValidationException(nameof(VerifyGooglePhoneRequest.TempToken), "Email Google đã được đăng ký.");
        }
