...
case "$VERIFY_DB_NAME" in
  SaigonWaterbusDb_verify_*) ;;
  *)
    echo "Refusing to verify against non-temporary database: $VERIFY_DB_NAME" >&2
    exit 1
    ;;
esac
...
dotnet run --project src/Web --no-build --no-launch-profile -- db:migrate-seed
